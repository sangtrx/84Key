...
# 3. Upstream diagnostic tracing must be scrubbed before the event tap starts.
...
# 4. Fork identity must not reuse upstream's TCC/code-sign identity.
...
# 5. GitHub Actions must be immutable-SHA pinned. A mutable @vN tag on code that
#    receives repository or artifact permissions is a supply-chain regression.
mutable_uses=$(grep -R -nE '^\s*- uses: [^@]+@[^#[:space:]]+' .github/workflows \
...
# 6. Release/build jobs execute only the checksum-pinned XcodeGen archive, not a
...
# 7. Signing credentials and contents:write must not coexist in one job.
#    Build job is explicitly read-only; publish job contains no secrets.* refs.
